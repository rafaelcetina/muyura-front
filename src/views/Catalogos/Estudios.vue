<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h3">Estudios</h1>
      </v-col>
      <v-col class="text-right">
        <v-btn color="success" @click="openNew()">Nuevo Estudio</v-btn>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
...
<script lang="ts">
import Vue from 'vue'
import {Estudio, _Estudio} from "@/models/Estudio";
import {CatalogoService} from "@/services/CatalogoService";
import FormCreate from "@/components/Catalogos/FormEstudioCreate.vue";
...
export default Vue.extend({
  ...
  data: () => ({
    dialog: false,
    service: new CatalogoService('estudios'),
    list: [] as _Estudio[],
    element: new Estudio(),
    search: '',
    headers: [
      {text: 'Id', value: 'id', align: 'left', width: 150},
      {text: 'Nombre', value: 'estudio_nombre', align: 'left'},
